...
const TwitterCtrl = {
  ...
  twitterUserInfo: async (expressRequest: Request, res: Response) => {
    try {
      ...
      if (!twitter)
        return res.status(500).json({ msg: "Sorry. We can't find your twitter account. Have you associated it in your User Settings page?" });
      ...
      const info = await User.findOneAndUpdate(
        ...
        {
          $push: { externalAccounts: { username: user.screen_name, provider: 'twitter', link: `https://www.twitter.com/${user.screen_name}` } },
  ...
  twitterLogout: async (expressRequest: Request, res: Response) => {
    try {
      ...
      const oauth_token = await User.findOneAndUpdate(
        ...
        { $pull: { tokens: { provider: 'twitter' }, externalAccounts: { provider: 'twitter' } } },
      );
