...
const TwitterCtrl = {
    ...
    twitterAccessToken: async (expressRequest: Request,res: Response) => {
        try {
            ...
            const {oauth_access_token, oauth_access_token_secret} = await twitterapis.oauth.getOauthAccessToken(oauth_token,oauth_token_secret,oauth_verifier)
    ...
    twitterUserInfo: async (expressRequest: Request,res: Response) => {
        try {
            ...
            const response = await twitterapis.oauth.getProtectedResource(`https://api.twitter.com/1.1/account/verify_credentials.json`, 'GET', oauth_access_token, oauth_access_token_secret)
    ...
    twitterGetUserPost: async (expressRequest: Request,res: Response) => {
        try {
            ...
            const response = await twitterapis.oauth.getProtectedResource(`https://api.twitter.com/1.1/lists/statuses.json?slug=${slug}&owner_screen_name=${owner_screen_name}&tweet_mode=extended&count=100`,'GET', oauth_access_token, oauth_access_token_secret);
    ...
    getHome: async (expressRequest: Request, res: Response) => {
        try {
            ...
            const twitter = user.tokens?.find((provider) => provider.provider === 'twitter');
            if (!twitter) return res.status(401).json({msg: 'You need to connect your twitter account to access this page!'})
            if (!twitter.oauth_access_token || !twitter.oauth_access_token_secret) return res.status(400).json({msg: "Please, try to login to twitter again!"});
            ...
            const data = JSON.parse(response.data) as TweetProps[];
            if (!Array.isArray(data)) return res.status(500).json({msg: "Invalid response from twitter!"})
