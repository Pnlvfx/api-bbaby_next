...
const TwitterCtrl = {
    ...
    twitterAccessToken: async (req:express.Request,res:express.Response) => {
        try {
            ...
            const saveTokens = await User.findOneAndUpdate({username: user?.username}, {tokens: {oauth_access_token: oauth_access_token, oauth_access_token_secret: oauth_access_token_secret}})
    ...
    twitterUserInfo: async (req:express.Request,res:express.Response) => {
        try {
            ...
            const info = await User.findOneAndUpdate({username: internalUser?.username}, {externalAccounts: [{username: user.screen_name, provider: 'twitter', link: `https://www.twitter.com/${user.screen_name}`}], hasExternalAccount: true})
