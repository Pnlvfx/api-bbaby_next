...
const postapis = {
  shareToTwitter: async (
    ...
    try {
      const govText =
        savedPost.title.substring(0, 300 - url.length - 10) + " " + url;
      const twitterText = user.role === 0 ? url : govText;
      if (!communityInfo.language)
        throw new Error("This community doesn't have a language");
      const twitterUser = twitterapis.chooseUser(
        user,
        savedPost,
        communityInfo.language
      );
      if (user.role === 1) {
        if (isImage || isVideo) {
          ...
          const media = isUrl
            ? await coraline.getMediaFromUrl(selectedFile, public_id, type)
            : { filename: selectedFile };
          const twimage = await twitterapis.uploadMedia(
            twitterUser,
            isUrl ? media.filename : Buffer.from(media.filename)
          );
